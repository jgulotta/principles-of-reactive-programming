...
package object nodescala {
  private[this] val NEVER_COMPLETES = new TimeoutException("this future never completes")
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
     ...
     */
    def never[T]: Future[T] = Future.failed(NEVER_COMPLETES)
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      val ab = new ArrayBuffer[T](fs.length)

      var i = 0
      for (f <- fs) {
        f onComplete(t => t match {
          case Success(t) => ab.update(i, t)
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = if (f.isCompleted) {
      ...
      throw new NoSuchElementException("cannot retrieve value from incomplete future")
    }
